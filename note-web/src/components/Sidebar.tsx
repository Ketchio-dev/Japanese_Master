...
import { Page, createPage, getWorkspacePages } from "@/lib/workspace";
...
export default function Sidebar({ workspaceId }: { workspaceId: string }) {
    ...
    const handleCreatePage = async (parentId: string | null = null) => {
        const newPage = await createPage(workspaceId, parentId);
        await loadPages(); // Refresh tree
        // If parent was closed, we might want to auto-expand it (omitted for MVP)
    ...
    return (
        <aside className="w-60 bg-gray-50 border-r border-gray-200 h-screen flex flex-col flex-shrink-0">
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-4">
                ...
                <div className="text-xs font-bold text-gray-400 px-2 py-1 mb-1 mt-4">PRIVATE</div>
                {rootPages.map(page => (
                    <PageItem
                        key={page.id}
                        page={page}
                ...
                <button
                    ...
                    className="mt-2 text-gray-500 hover:text-black hover:bg-gray-200 w-full text-left px-2 py-1 rounded text-sm flex items-center gap-2"
                >
            ...
            <div className="p-2 border-t border-gray-200">
                <div className="flex items-center gap-2 p-2 hover:bg-gray-200 rounded cursor-pointer text-sm text-gray-600">
                    <Settings size={16} />
